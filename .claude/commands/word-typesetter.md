---
description: 交互式学术论文 Word 自动排版——参数前置收集或接收子 Agent 分析结果，然后一键执行
---

# Word Auto Typesetter

交互式收集论文排版要求（或接收 template-analyzer 子 Agent 的分析结果），通过 Python `win32com.client` 自动修改 Word 文档格式。
**核心原则：只改格式，不改内容。**
...
用户直接调用 `/word-typesetter`，输出 21 类问卷，用户填写参数后执行排版。

### 模式二：接收子 Agent 参数（有模板时）

**完整流程**：

1. 用户 `@待排版文件 @模板文件 排版`
2. 先调用 template-analyzer 子 Agent，输出：
   - 参数汇总（自然语言，如"正文字体：宋体，字号：小四，行距：1.25倍"）
   - 标记指南
   - 未覆盖项（模板有要求但 word-typesetter 标准步骤不支持的项目）
3. 用户确认参数
4. word-typesetter 主 Agent **不再输出问卷**，按以下规则生成排版脚本

**参数填充规则**：

```
子 Agent 提供了值的参数  →  直接填入代码模板，跳过该项的默认值
子 Agent 未提供的参数    →  使用 word-typesetter 默认值
子 Agent 标注"需确认"的  →  单独向用户确认后再填入
```

**参数解析映射（子 Agent 自然语言 → 代码常量）**：

| 子 Agent 输出示例 | 代码映射 |
|------------------|---------|
| 行距：单倍 | `wdLineSpaceSingle(0)` |
| 行距：1.5倍 | `wdLineSpace1pt5(1)` |
| 行距：1.25倍 | `wdLineSpaceMultiple(5)`, `LineSpacing=1.25` |
| 行距：固定值 20 磅 | `wdLineSpaceExactly(4)`, `LineSpacing=20` |
| 字号：小四 | `12`（查字号对照表） |
| 对齐：两端对齐 | `3`（wdAlignParagraphJustify） |
| 首行缩进：2 字符 | `FirstLineIndent = 2 * 字号磅值` |
| 段前：0.5 行 | `SpaceBefore = 0.5 * 字号磅值` |
| 纸张：A4 | `7`（wdPaperA4） |
| 边距：2.5cm | `2.5 * 28.35 = 70.9` 磅 |
| 页眉距离：1.6cm | `1.6 * 28.35 = 45.4` |

**未覆盖项处理规则**：

子 Agent 列出的每条未覆盖项，主 Agent 在 19 步模板的基础上**追加新的处理步骤**。生成规则：

1. 阅读子 Agent 对该项的具体描述（包括来源批注原文）
2. 理解其格式要求，翻译为 Word COM 操作
3. 在排版脚本的 19 步之后插入新步骤，命名 `Step 20/21/22...`
4. 新步骤遵循同样的安全红线（try/except、不删内容）

示例：如果子 Agent 输出"公式居中+编号右对齐"：
```python
# === Step 20: 公式处理（模板要求，子 Agent 提取） ===
for para in doc.Paragraphs:
    if "《公式》" in para.Range.Text:
        # 公式居中
        para.Range.ParagraphFormat.Alignment = 1  # wdAlignCenter
        # 编号右对齐：在公式后插入右对齐制表位
        para.Range.ParagraphFormat.TabStops.Add(
            Position=page_width - margins,
            Alignment=2  # wdAlignTabRight
        )
```
...
```
最终参数 = word-typesetter 默认值
         ∪ 子 Agent 提供的值（覆盖默认）
         ∪ 用户手动确认的值（最高优先级）

最终步骤 = word-typesetter 标准 19 步
         ∪ 子 Agent 未覆盖项生成的新步骤
```
