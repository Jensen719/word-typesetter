...
当 template-analyzer 子 Agent 已从模板中提取完参数后，word-typesetter 跳过问卷，直接读取子 Agent 输出的参数。

流程：
1. 用户 @待排版文件 @模板文件 说"排版"
2. 先调用 template-analyzer 子 Agent 分析模板
3. 子 Agent 输出参数汇总 + 标记指南 + 未覆盖项
4. 用户确认参数无误
5. word-typesetter 主 Agent 读取参数，生成排版脚本
6. 对子 Agent 列出的"未覆盖项"，主 Agent 在 19 步基础上新增对应处理步骤
7. 执行排版

**关键规则**：
- 子 Agent 输出的参数优先于默认值
- 子 Agent 列出的未覆盖项需要在脚本中新增步骤，不能忽略
- 如果某个参数子 Agent 标注了"需确认"，主 Agent 单独向用户确认
